...
# 3. 환경 변수 설정
cp .env.example .env
# .env를 열어 실제 API 키와 Elasticsearch endpoint를 입력

# 4. Streamlit 실행
streamlit run app/streamlit_app.py
```

Pinecone 인덱스가 없으면 앱이 처음 적재할 때 4096차원 cosine 인덱스를 자동 생성합니다.
직접 생성할 경우에도 dimension은 `4096`, metric은 `cosine`으로 맞춰야 합니다.

## 테스트

외부 API를 실제 호출하지 않는 단위 테스트가 포함되어 있습니다.
